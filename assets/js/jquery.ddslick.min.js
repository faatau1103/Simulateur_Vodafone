(function(a) {
    ...
    function j(a) {
        ...
        if (c.length <= 0 && d.length > 0) {
            a.find(".dd-selected-text").css("lineHeight", 1)
        }
    }
    ...
    function k(b) {
        b.find(".dd-option").each(function() {
            ...
            if (e.length <= 0 && f.length > 0) {
                c.find(".dd-option-text").css({"lineHeight": 1,"margin-top":3+"px"})
            }
        })
    }
    ...
    var b = {},
        ...
        d = '<div class="dd-select"><input class="dd-selected-value" type="hidden" name="data[Publication][privacy]" /><a class="dd-selected"></a><span class="dd-pointer dd-pointer-down"></span></div>',
        e = '<ul class="dd-options"></ul>',
        f = '<style id="css-ddslick" type="text/css">' + ".dd-select{ position:relative; cursor:pointer;}" + ".dd-desc { color:#aaa; display:block; overflow: hidden; font-weight:normal; line-height: 1.4em; }" + ".dd-selected{ overflow:hidden; display:block; padding:10px; font-weight:bold;padding-left:2px;}" + ".dd-pointer{ width:0; height:0; position:absolute; right:10px; top:50%; margin-top:-3px;}" + ".dd-pointer-down{ border:solid 5px transparent; border-top:solid 5px #000; }" + ".dd-pointer-up{border:solid 5px transparent !important; border-bottom:solid 5px #000 !important; margin-top:-8px;}" + ".dd-options{ border:solid 2px #000; border-top:none; list-style:none;  display:none; position:absolute; z-index:2000; margin:0 auto; padding:0;background:#fff; overflow:auto;line-height:1px;top: 50px;max-width: 137px; float: none; left: 0px; right: 0px;}" + ".dd-option{ padding:0px; display:block;  overflow:hidden; text-decoration:none; color:#333; cursor:pointer;-webkit-transition: all 0.25s ease-in-out; -moz-transition: all 0.25s ease-in-out;-o-transition: all 0.25s ease-in-out;-ms-transition: all 0.25s ease-in-out;margin-top:0px; }" + ".dd-options > li:last-child > .dd-option{ border-bottom:none;}" + ".dd-option:hover{ background:#000; color:#fff;}" + ".dd-selected-description-truncated { text-overflow: ellipsis; white-space:nowrap; }" + ".dd-option-selected { background:#f6f6f6; }" + ".dd-option-image, .dd-selected-image { vertical-align:middle; float:left; margin-right:5px; max-width:64px;width:16px;height:16px;    margin-top: 2px;}" + ".dd-image-right { float:right; margin-right:15px; margin-left:5px;}" + ".dd-container{ position:relative;}​ .dd-selected-text { font-weight:bold}​</style>";
